/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ReceiverWhereUniqueInput } from "./ReceiverWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ReceiverUpdateInput } from "./ReceiverUpdateInput";

@ArgsType()
class UpdateReceiverArgs {
  @ApiProperty({
    required: true,
    type: () => ReceiverWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ReceiverWhereUniqueInput)
  @Field(() => ReceiverWhereUniqueInput, { nullable: false })
  where!: ReceiverWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ReceiverUpdateInput,
  })
  @ValidateNested()
  @Type(() => ReceiverUpdateInput)
  @Field(() => ReceiverUpdateInput, { nullable: false })
  data!: ReceiverUpdateInput;
}

export { UpdateReceiverArgs as UpdateReceiverArgs };
